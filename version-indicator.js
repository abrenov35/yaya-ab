(function(){
  ...
  function removeLegacyVersions(){
    ...
    header.querySelectorAll('span,.sync').forEach(function(el){
      ...
      if(/^v\d+(?:\.\d+)+$/i.test(text)||/^v\d+$/i.test(text))el.remove();
    });
  }
  ...
  function installStyle(){
    ...
    style.textContent=`
      ...
      @media(max-width:760px){
        .hdr .tabs #${BADGE_ID}{display:inline-flex!important;font-size:9.5px!important;height:20px!important;padding:0 6px!important;margin-left:4px!important;}
      }
    `;
  ...
  function show(version){
    removeLegacyVersions();
    installStyle();
    const display=visibleVersion(version);
    const title='Yaya v'+display+' — AB RENOV 35';
    document.title=title;
    document.documentElement.setAttribute('data-yaya-version',display);
    document.documentElement.setAttribute('data-yaya-build-version',String(version||''));

    const header=document.querySelector('.hdr');
    const tabs=header&&header.querySelector('.tabs');
    if(!header||!tabs){setTimeout(function(){show(version);},120);return;}
    ...
    if(badge.parentElement!==tabs)tabs.appendChild(badge);
    else tabs.appendChild(badge);
    removeLegacyVersions();
  }
