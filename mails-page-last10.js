(function(){
  ...
  function getMails(){try{const list=typeof window.yayaMailRows==='function'?window.yayaMailRows():(S.documents||[]);return list.map((d,i)=>({d,i})).filter(x=>String(x.d.type||'').toLowerCase()==='mail').sort((a,b)=>ordreMail(b.d,b.i)-ordreMail(a.d,a.i)||b.i-a.i).slice(0,10).map(x=>x.d);}catch(e){return [];}}
