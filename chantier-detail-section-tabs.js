(function(){
  ...
  function ensureMailPane(card,tabs,rows){
    ...
    pane.dataset.empty=rows.length?'0':'1';
    const displayRows=rows.map(row=>{
      const name=typeof nomMailYaya==='function'
        ? nomMailYaya(row)
        : String(row.nomMail||row.expediteur||row.from||row.sujet||'Expéditeur non renseigné').replace(/<.*$/,'').trim();
      const subject=typeof objetMailYaya==='function'
        ? objetMailYaya(row)
        : String(row.objetMail||row.mailSubject||row.emailSubject||row.subject||row.objet||'Objet non renseigné').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim();
      const date=String(row.date||'').slice(0,10).split('-').reverse().join('/');
      return {row,name,subject,date};
    });
    const html=displayRows.map(item=>
      '<div class="yaya-detail-document-row yaya-detail-mail-row">'
        +'<strong class="yaya-mail-sender" title="'+escapeHtml(item.name)+'">'+escapeHtml(item.name)+'</strong>'
        +'<span class="yaya-mail-subject" title="'+escapeHtml(item.subject)+'">'+escapeHtml(item.subject)+'</span>'
        +'<span class="yaya-detail-charge-cost">'+escapeHtml(item.date||'—')+'</span>'
    ...
    const signature=JSON.stringify(displayRows.map(item=>[item.row.id||'',item.name,item.subject,item.date]));
    if(pane._yayaRowsSignature!==signature){
      pane.innerHTML=html;
      pane._yayaRowsSignature=signature;
    }
