...
function showDoc(cid,i){const a=docsFor(cid);if(!a.length)return renderViewer(cid);i=Math.max(0,Math.min(Number(i)||0,a.length-1));document.querySelectorAll('#yayaDevisViewer .ydd-tab').forEach((b,n)=>b.classList.toggle('on',n===i));const v=document.querySelector('#yayaDevisViewer .ydd-view');if(v)v.innerHTML='<iframe class="ydd-frame" title="Visualisation du devis" src="'+esc(a[i].url)+'"></iframe>';}
...
css();const pane=document.getElementById('pane-chantiers');if(pane&&!pane.dataset.yayaDevisDocsDelegated){pane.dataset.yayaDevisDocsDelegated='1';pane.addEventListener('click',e=>{const t=e.target.closest('[data-yaya-devis-docs="1"]');if(!t)return;e.preventDefault();e.stopPropagation();openViewer(t.dataset.yayaChantierId||cardId(t.closest('.card')));},true);pane.addEventListener('keydown',e=>{if(e.key!=='Enter'&&e.key!==' ')return;const t=e.target.closest('[data-yaya-devis-docs="1"]');if(!t)return;e.preventDefault();openViewer(t.dataset.yayaChantierId||cardId(t.closest('.card')));},true);}decorate();ensureCentral().catch(()=>{});let q=false;new MutationObserver(()=>{if(q)return;q=true;requestAnimationFrame(()=>{q=false;decorate();});}).observe(document.documentElement,{childList:true,subtree:true});window.yayaOpenDevisDocuments=openViewer;
})();
