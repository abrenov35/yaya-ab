(function(){
  ...
  function isVisualizationModal(modal){if(!modal)return false;if(modal.classList.contains('yaya-mail-body-modal')||modal.classList.contains('message-modal'))return false;if(modal.classList.contains('piece-preview-modal')||modal.classList.contains('yaya-document-read-modal'))return true;if(modal.querySelector('.piece-preview-stage,.piece-pdf-pages,.piece-image-stage,.piece-drive-pages-stage'))return true;const hasViewer=!!modal.querySelector('iframe,embed,object,video,img');const hasEditor=!!modal.querySelector('input:not([type="hidden"]),textarea,select');if(hasViewer&&!hasEditor)return true;const title=String(modal.querySelector('h5,h4,h3')?.textContent||'').trim();return /^(aperçu|visualisation|lecture|pi[eè]ce jointe|document|[ée]change chantier)/i.test(title)&&!hasEditor;}
