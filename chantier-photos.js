(function(){
'use strict';
if(window.__YAYA_PHOTOS_V31)return;window.__YAYA_PHOTOS_V31=true;
...
async function movePhotoIntoGroup(photo,key){
  ...
  var cid=String(photo.chantierId||''),list=photoGroupRows(cid,key);
  ...
  queuePhotoUpsert(photo);savePhotoCache();refresh();
...
function movePhotoToGroup(photo){
  ...
  r.querySelector('.sv').onclick=async function(){
    ...
    close();await movePhotoIntoGroup(photo,key);
  };
}
...
async function openPhotoReview(cid,files,mode){
  ...
  for(var f of Array.from(files||[])){
    ...
    var fromName=photoDateFromFilename(originalName);
...
var dragPhotoId='',dragPhotoCard=null,touchDrag=null,suppressPhotoOpenUntil=0;
function dragTarget(x,y){
  var el=document.elementFromPoint(x,y),group=el&&el.closest('.yaya-pg[data-photo-group]');
  return group&&dragPhotoCard&&group.closest('.card')===dragPhotoCard?group:null;
}
