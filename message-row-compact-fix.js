...
function looksLikeBody(v){
 ...
 if(/\b(Bonjour|Bonsoir|Cordialement|Merci|Bien à vous|a écrit|De\s*:|From\s*:|Envoyé\s*:|Sent\s*:|Téléphone|www\.)\b/i.test(t)&&t.length>90)return true;
 return false;
}
function subjectFor(d){
 const vals=[d&&d.subject,d&&d.objet,d&&d.mailSubject,d&&d.emailSubject,d&&d.intitule];
 for(const v of vals){
   const t=String(v||'').replace(/\s+/g,' ').trim();
   if(t&&!looksLikeBody(t))return t.replace(/^objet\s*:\s*/i,'');
 }
...
function compact(root){
 (root||document).querySelectorAll('.message-ligne').forEach(row=>{
   ...
   if(a){
     let d=null;
     const action=row.querySelector('[onclick*="voirMessageYaya"]');
     const raw=String(action&&action.getAttribute('onclick')||'');
     const m=raw.match(/voirMessageYaya\(['\"]([^'\"]+)/);
     try{if(m&&window.S&&Array.isArray(S.documents))d=S.documents.find(x=>String(x.id)===String(m[1]))||null;}catch(e){}
