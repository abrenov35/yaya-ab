(function(){
  'use strict';
  if(window.__yayaMailEditBackgroundSaveV3)return;
  window.__yayaMailEditBackgroundSaveV3=true;
  ...
  function setObjectFields(d,objet){d.objetMail=objet;d.mailSubject=objet;d.emailSubject=objet;d.subject=objet;d.objet=objet;d.messageSubject=objet;d.gmailSubject=objet;d.subjectMail=objet;d.titreMail=objet;d.intitule=objet;}
  ...
  window.saveDocumentEdit=function(id){
    ...
    d.chantierId=String(ch.value||'');d.type=String(type.value||'Mail');d.sujet=exp;d.nomMail=exp;d.origineMail='MAIL';setObjectFields(d,objet);
    ...
    try{window.dispatchEvent(new CustomEvent('yaya:mail-edit-local',{detail:{id:String(d.id),subject:objet}}));}catch(e){toastSafe('Mail non synchronisé — réessayer',true);}
  };
})();
