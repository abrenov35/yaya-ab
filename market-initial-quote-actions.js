(function(){
  ...
  if(!document.getElementById(STYLE_ID)){
    ...
    style.textContent=`
      ...
      .yaya-devis-document-field .yaya-devis-document-view-inline,.yaya-devis-document-field .yaya-devis-fast-replace{min-height:34px!important;padding:6px 10px!important;border-radius:7px!important;font-size:11.5px!important;font-weight:750!important;box-shadow:none!important;white-space:nowrap!important;border:1px solid #a9c8e8!important;background:#f3f8fd!important;color:#174d7d!important}
      ...
      @media(max-width:640px){.yaya-devis-document-field .yaya-devis-fast-piece{align-items:center!important;gap:8px!important;padding:9px!important}.yaya-devis-document-actions{gap:6px!important}.yaya-devis-document-field .yaya-devis-document-view-inline,.yaya-devis-document-field .yaya-devis-fast-replace{padding:6px 9px!important;font-size:11px!important}}
    `;
  ...
  function confirmDelete(c){
    ...
    overlay.className='yaya-devis-delete-overlay';
    overlay.innerHTML='<div class="yaya-devis-delete-box" role="dialog" aria-modal="true"><div style="font-size:30px;margin-bottom:8px">🗑️</div><h3>Supprimer le devis 1 ?</h3><p>Le devis 1 sera retiré de Yaya. Le montant du marché HT restera strictement inchangé. Le fichier source Drive / OneDrive n’est pas supprimé.</p><div class="yaya-devis-delete-actions"><button type="button" class="yaya-devis-delete-cancel">Annuler</button><button type="button" class="yaya-devis-delete-ok">Supprimer</button></div></div>';
    ...
    overlay.querySelector('.yaya-devis-delete-ok').onclick=async function(){
      ...
      try{
        ...
        toastSafe('Devis 1 supprimé ✓');
      }catch(e){
        c.montantDevisHT=before.montantDevisHT;c.notes=before.notes;renderSafe();schedule();toastSafe('Suppression du devis impossible',true);
      }
    };
  }
  ...
  function decorateRows(){
    document.querySelectorAll('#pane-chantiers .yaya-detail-market-row').forEach(row=>{
      ...
      row.dataset.yayaInitialDeleted='0';
      row.style.removeProperty('display');
      let del=row.querySelector('.yaya-initial-devis-delete');
      if(!del){
        del=document.createElement('button');del.type='button';del.className='yaya-detail-document-delete yaya-initial-devis-delete';del.textContent='🗑️';del.title='Supprimer le devis 1';del.setAttribute('aria-label','Supprimer le devis 1');
        ...
        del.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const cc=chantier(id);if(cc)confirmDelete(cc);});
      }
      syncEmpty(row.closest('.yaya-detail-markets-pane'));
    });
  }
  ...
  function currentContext(modal){
    ...
    if(modal&&modal.querySelector('#edNom')){try{if(typeof focusChantier!=='undefined'&&focusChantier)return {kind:'main',id:String(focusChantier)};}catch(e){}}
    return {kind:'',id:''};
  }

  function rowLink(kind,id){
    for(const row of document.querySelectorAll('#pane-chantiers .yaya-detail-market-row')){
      const edit=row.querySelector('.yaya-detail-document-edit');if(!edit)continue;
      if(String(edit.dataset.kind||'')!==String(kind||'')||String(edit.dataset.rowId||'')!==String(id||''))continue;
      const view=row.querySelector('.yaya-detail-document-view');const link=http(view&&view.dataset&&view.dataset.lien);if(link)return link;
    }
    return '';
  }

  function quoteLink(kind,id){
    if(kind==='main'){const c=chantier(id);if(isDeleted(c))return '';const direct=http(c&&c.notes);if(direct)return direct;}
    else if(kind==='avenant'){const v=avenant(id);const direct=http(v&&v.lien);if(direct)return direct;}
  ...
  function ensureDocumentField(){
    ...
    if(!zone){zone=modal.querySelector('.yaya-devis-fast-piece');if(zone)zone.id='pj-zone';}
    if(!zone){const foot=modal.querySelector('.yaya-devis-fast-foot');if(!foot)return;zone=document.createElement('div');zone.className='yaya-devis-fast-piece';zone.id='pj-zone';foot.insertAdjacentElement('beforebegin',zone);}
    let wrapper=zone.closest('.yaya-devis-document-field');
    if(!wrapper){wrapper=document.createElement('div');wrapper.className='yaya-devis-document-field';const label=document.createElement('div');label.className='yaya-devis-document-label';label.textContent='Document';zone.parentNode.insertBefore(wrapper,zone);wrapper.appendChild(label);wrapper.appendChild(zone);}
    if(zone.classList.contains('scan-zone')||zone.classList.contains('scan-ok'))return;
    const link=quoteLink(ctx.kind,ctx.id);
    let oldView=zone.querySelector('#yayaFastPieceBtn');const hasFile=!!link||!!oldView;
    let status=zone.querySelector('.yaya-devis-document-status');if(!status){status=document.createElement('span');status.className='yaya-devis-document-status';zone.insertBefore(status,zone.firstChild);}status.textContent=hasFile?'Document joint':'Aucun document joint';
    let actions=zone.querySelector('.yaya-devis-document-actions');if(!actions){actions=document.createElement('span');actions.className='yaya-devis-document-actions';zone.appendChild(actions);}
    ...
    if(hasFile&&link&&!inlineView){inlineView=document.createElement('button');inlineView.type='button';inlineView.className='yaya-devis-document-view-inline';inlineView.textContent='Voir';inlineView.title='Voir le document';actions.appendChild(inlineView);}
    if(!hasFile&&inlineView)inlineView.remove();
    if(inlineView&&link&&!inlineView._yayaDocumentViewBound){inlineView._yayaDocumentViewBound=true;inlineView.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(typeof voirPiece==='function')voirPiece(link);else window.open(link,'_blank','noopener');});}
    let btn=actions.querySelector('.yaya-devis-fast-replace');if(!btn){btn=document.createElement('button');btn.type='button';btn.className='yaya-devis-fast-replace';actions.appendChild(btn);}
    btn.textContent=hasFile?'🔄 Remplacer':'📎 Ajouter';btn.title=hasFile?'Remplacer le document':'Ajouter un document';btn.setAttribute('aria-label',btn.title);
    btn.onclick=function(e){e.preventDefault();e.stopPropagation();if(typeof remplacerPJ==='function')remplacerPJ('devis',ctx.id);else toastSafe('Ajout de document indisponible',true);};
  }
