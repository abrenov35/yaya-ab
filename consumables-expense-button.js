(function(){
  ...
  function installStyle(){
    ...
    style.id=STYLE_ID;
    style.textContent=`
      #pane-chantiers .yaya-consumables-button{
        height:34px!important;min-height:34px!important;min-width:126px!important;
        padding:3px 10px!important;display:flex!important;flex-direction:column!important;
        align-items:center!important;justify-content:center!important;gap:1px!important;
        background:#fff8ec!important;border-color:#e9cf9c!important;color:#7a560e!important;
      ...
      #pane-chantiers .yaya-consumables-button small{font-size:9.5px!important;font-weight:650!important;opacity:.65!important;white-space:nowrap!important}
      ...
      .yaya-consumables-summary span{font-size:11px!important;color:#64748b!important}
      .yaya-consumables-summary b{font-size:19px!important}
      .yaya-consumables-summary small{grid-column:1/-1!important;font-size:10.5px!important;color:#7a8798!important}
      ...
      @media(max-width:640px){
        #pane-chantiers .yaya-consumables-button{min-width:112px!important;padding-left:7px!important;padding-right:7px!important}
        #pane-chantiers .yaya-consumables-button strong{font-size:10px!important}
        #pane-chantiers .yaya-consumables-button small{font-size:8.8px!important}
        .yaya-consumables-choices{grid-template-columns:repeat(3,1fr)!important}
      }
    `;
  ...
  function makeId(){
    ...
    return 'CONSOMMABLES_'+Date.now()+'_'+Math.random().toString(36).slice(2,8);
  }
  ...
  function chantierName(id){
    try{
      const c=Array.isArray(S&&S.chantiers)?S.chantiers.find(function(x){return String(x&&x.id||'')===String(id);}):null;
  ...
  function hoursFor(cid){
    try{
      ...
      return heures.reduce(function(total,h){
        if(!h||h.type!=='chantier'||String(h.ref)!==String(cid))return total;
        const salarie=salaries.find(function(s){return String(s&&s.id||'')===String(h.salarieId||'');});
  ...
  function findExpense(cid){
    try{
      const achats=Array.isArray(S&&S.achats)?S.achats:[];
      return achats.find(function(a){
        if(!a||String(a.chantierId||'')!==String(cid))return false;
        return String(a.typeDoc||'')===TYPE || String(a.fournisseur||'')===FOURNISSEUR || String(a.designation||'').includes(MARKER);
      })||null;
    }catch(e){return null;}
  }

  function validCoeff(value){
    const n=Number(value);
    return COEFFS.some(function(x){return Math.abs(x-n)<0.001;})?n:null;
  }
  ...
  function coefficientFor(cid){
    ...
    try{
      const n=validCoeff(Number(localStorage.getItem(coeffKey(cid))));
      if(n!=null)return n;
    }catch(e){}
  ...
  function persistSoon(now){
    if(syncTimer)clearTimeout(syncTimer);
    syncTimer=setTimeout(async function(){
      syncTimer=null;
      try{if(typeof apiPost==='function')await apiPost('setAchats',Array.isArray(S&&S.achats)?S.achats:[]);}catch(e){console.warn('Consommables non synchronisés',e);}
    },now?0:800);
  }
  ...
  function ensureExpense(cid,forceCreate){
    ...
    const coeff=coefficientFor(cid);
    const amount=amountFor(cid,coeff);
    ...
    if(!expense){
      ...
      const des=designationFor(cid,coeff);
    ...
    if(changed){saveCache();persistSoon(false);}
    return changed;
  }
  ...
  function closeModal(){document.querySelectorAll('.'+MODAL_CLASS).forEach(function(x){x.remove();});}

  function openModal(cid){
    installStyle();
    closeModal();
    if(!cid){toastSafe('Chantier introuvable',true);return;}
    const coeff=coefficientFor(cid);
    const h=hoursFor(cid);
    const amount=amountFor(cid,coeff);
    ...
    overlay.querySelectorAll('.yaya-consumables-choice').forEach(function(btn){
      btn.addEventListener('click',function(){
        const selected=validCoeff(Number(btn.dataset.coeff));
        ...
        closeModal();
        scheduleRender();
        toastSafe('Coefficient consommables : '+selected.toLocaleString('fr-FR',{minimumFractionDigits:1,maximumFractionDigits:1})+' ✓');
      });
    });
  }

  function updateButton(button,cid){
    const coeff=coefficientFor(cid);
    const amount=amountFor(cid,coeff);
    const coeffLabel=coeff.toLocaleString('fr-FR',{minimumFractionDigits:1,maximumFractionDigits:1});
    const signature=amount+'|'+coeffLabel;
    if(button.dataset.signature===signature)return;
    button.dataset.signature=signature;
    button.innerHTML='<strong>Consommables : '+esc(euro(amount))+'</strong><small>coef. '+esc(coeffLabel)+'</small>';
  }

  function decorateRow(row){
    if(!row||row.dataset.section!=='depenses')return;
    const group=row.querySelector('.yaya-stock-action-buttons');
    if(!group)return;
    ...
    if(!button){
      ...
      button.type='button';
      button.className='yaya-detail-section-action-button yaya-consumables-button';
      button.title='Modifier le coefficient consommables';
      const stock=group.querySelector('.yaya-stock-expense-button');
      if(stock)group.insertBefore(button,stock);else group.appendChild(button);
      button.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();openModal(cid);});
    }
